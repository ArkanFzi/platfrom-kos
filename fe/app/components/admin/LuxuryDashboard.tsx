    ...
    <div className="p-4 md:p-8 space-y-6 md:space-y-8 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm md:text-base">Welcome back, Administrator</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="px-3 py-1.5 md:px-4 md:py-2 bg-slate-800/50 border border-slate-700 rounded-lg">
            <p className="text-[10px] md:text-xs text-slate-500 uppercase tracking-wider">Last updated</p>
            <p className="text-xs md:text-sm text-white font-medium">Just now</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 md:gap-6">
        ...
        <div className="group relative overflow-hidden bg-gradient-to-br from-blue-500/10 to-blue-600/10 border border-blue-500/20 rounded-2xl p-4 md:p-6 hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-300">
          ...
          <div className="relative">
            ...
            <p className="text-slate-400 text-[10px] md:text-sm mb-1">Active Tenants</p>
            <p className="text-xl md:text-3xl font-bold text-white mb-0.5 md:mb-1">{activeTenants}</p>
            <p className="text-[10px] text-slate-500">{tenants.length} total registrations</p>
          </div>
        </div>
        ...
        <div className="group relative overflow-hidden bg-gradient-to-br from-green-500/10 to-green-600/10 border border-green-500/20 rounded-2xl p-4 md:p-6 hover:shadow-2xl hover:shadow-green-500/10 transition-all duration-300">
          ...
          <div className="relative">
            ...
            <p className="text-slate-400 text-[10px] md:text-sm mb-1">Available Rooms</p>
            <p className="text-xl md:text-3xl font-bold text-white mb-0.5 md:mb-1">{availableRooms}</p>
            <p className="text-[10px] text-slate-500">Out of {roomsCount} rooms</p>
          </div>
        </div>
          ...
          <div className="relative">
            <div className="flex items-start justify-between mb-2 md:mb-4">
              ...
              <div className="px-1.5 md:px-2 py-0.5 md:py-1 bg-red-500/20 rounded-lg">
                <span className="text-[10px] text-red-400 font-medium">Action Req.</span>
              </div>
            </div>
            <p className="text-slate-400 text-[10px] md:text-sm mb-1">Pending Payments</p>
            <p className="text-xl md:text-3xl font-bold text-white mb-0.5 md:mb-1">{pendingPayments}</p>
            ...
            {tenants.length > 0 ? tenants.slice(0, 4).map((tenant) => (
              <div key={tenant.id} className="flex items-center gap-4 p-4 bg-slate-800/30 border border-slate-700/50 rounded-xl hover:bg-slate-800/50 transition-all duration-200">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-white truncate">{tenant.nama_lengkap || tenant.user?.username || 'Guest'}</p>
                  <p className="text-sm text-slate-400 truncate">
                    {tenant.kamar?.nomor_kamar ? `Kamar ${tenant.kamar.nomor_kamar}` : (tenant.user?.username ? `@${tenant.user.username}` : 'New User')}
                  </p>
                </div>
                <span className={`px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap border ${tenant.status === 'Active' || !tenant.status
