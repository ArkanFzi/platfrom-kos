    ...
    const fetchTenants = async () => {
      ...
      try {
        ...
        const mapped = data.map((t: BackendTenant) => ({
          id: String(t.id),
          name: t.nama_lengkap || t.user?.username || 'Guest',
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 md:space-y-8 bg-slate-950 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-amber-500">Tenant Data</h2>
          <p className="text-slate-400 text-xs md:text-sm">Manage tenant information and history</p>
        </div>
      ...
      <div className="bg-slate-900/20 md:border md:border-slate-800 rounded-[1.5rem] md:rounded-[2rem] p-4 md:p-8 backdrop-blur-sm">
        {/* Desktop Header */}
        <div className="hidden md:grid grid-cols-[80px_1.5fr_1.5fr_1fr_1fr_1fr_1fr] gap-4 px-6 mb-8 text-slate-500 text-xs font-bold uppercase tracking-widest">
          <div>ID</div>
          <div>Tenant Name</div>
        ...
        <div className="space-y-4">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="text-slate-500 italic">Retrieving tenant records...</p>
            </div>
            ...
            filteredTenants.map((tenant) => (
              <div key={tenant.id}>
                {/* Desktop Row */}
                <div className="hidden md:grid grid-cols-[80px_1.5fr_1.5fr_1fr_1fr_1fr_1fr] gap-4 px-6 py-4 items-center bg-slate-800/20 hover:bg-slate-800/40 rounded-2xl transition-all border border-transparent hover:border-slate-800 group">
                ...
                <div className="md:hidden p-5 bg-slate-900/40 border border-slate-800 rounded-2xl space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-800">
                    <div>
                      <p className="text-slate-500 text-[10px] uppercase font-bold tracking-widest mb-1">Room</p>
                      <p className="text-amber-500 text-sm font-bold">{tenant.roomName}</p>
                    </div>
                    <div>
                      <p className="text-slate-500 text-[10px] uppercase font-bold tracking-widest mb-1">Check In</p>
                      <p className="text-white text-sm">{tenant.checkIn}</p>
                    </div>
                  </div>

                  <div className="pt-2">
                    <p className="text-slate-500 text-[10px] uppercase font-bold tracking-widest mb-1">Email / Phone</p>
